...
describe("getSmaDataPoint should behave correctly", () => {
    ...
    describe("The function should validate it's parameters", () => {
        ...
        it("should throw an error if ticker is not a string", () => { 
            expect(badTicker).toThrow(Error);
        })
        
        it("should throw an error if date is not a string in the format expected", () => { 
            expect(badDate).toThrow(Error);
        })
        
        it("should throw an error if days is not an integer", () => { 
            expect(badDays).toThrow(Error);
        })
    })
    ...
    describe("The function should return an object with a specific format", () => {
        ...
        const goodRecievedKeys = new Set(Object.keys(goodKeys()))

        it("should have only the keys expected", () => { 
            expect(areSetsEqual(expectedKeys, goodRecievedKeys)).toBeTruthy();
        })
        
        describe("should have values that are the right type and format", () => { 
            it("should have valid ticker", () =>{
                const ticker = goodKeys()["ticker"];
                expect(typeof ticker === 'string' || ticker instanceof String).toBeTruthy();
            })

            it("should have valid date", () =>{
                const date = goodKeys()["date"];
            ...
            it("should have valid sma", () =>{
                const sma = goodKeys()["sma"];
    ...
    describe("The function should return the average for the stock given the interval and date", () => {
        // Step 3 in mocking, mock the return value of the mocked function
        ...
        it("should have correct SMA 24 for mocked API response", () => { 
            expect(sma24["sma"]).toBeCloseTo(sma24AvgValue);
        })
        it("should have correct SMA 12 for mocked API response", () => { 
            expect(sma12["sma"]).toBeCloseTo(sma12AvgValue);
        })
    })
})